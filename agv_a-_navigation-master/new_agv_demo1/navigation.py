...
class Planner:
    ...
    def run(self):
        for task in self.tasks:
            plan = self.choose_agv_for_task(task)
            self.commit_plan(task, plan)

            print(
                f"AGV{plan['agv_id']} -> "
                f"{task['task_id']} "
                f"{task['destination_label']} "
                f"pickup={plan['pickup']['label']} "
                f"unload={plan['unload_pos']} "
                f"delivery={plan['delivery_time']} "
                f"delay={plan['delay']}"
            )
